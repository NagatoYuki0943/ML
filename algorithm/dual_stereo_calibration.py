...
class DualStereoCalibration:
    def __init__(
        ...
        self.camera_matrix_left = np.array(camera_matrix_left) if camera_matrix_left is not None else None
        self.camera_matrix_right = np.array(camera_matrix_right) if camera_matrix_right is not None else None
        self.distortion_coefficients_left = np.array(distortion_coefficients_left) if distortion_coefficients_left is not None else None
        self.distortion_coefficients_right = np.array(distortion_coefficients_right) if distortion_coefficients_right is not None else None
...
if __name__ == "__main__":
    ...
    xy = [200.0, 200.0]
    # left 畸变矫正后的坐标点
    xy_corrected = dual_stereo_calibration.undistort_point(xy, camera_matrix_left, R1, P1)
    print(f"left 矫正后的点坐标为: {xy_corrected}")

    # right 畸变矫正后的坐标点
    xy_corrected = dual_stereo_calibration.undistort_point(xy, camera_matrix_right, R2, P2)
    ...
    # 畸变矫正后的坐标点
    xy_corrected = dual_stereo_calibration.undistort_points(xy, camera_matrix_left, R1, P1)
    print(f"left 矫正后的点坐标为: {xy_corrected}")
