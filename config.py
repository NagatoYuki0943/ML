...
class AdjustCameraConfig(BaseConfig):
    ...
    lock = Lock()
    mean_light_suitable_range: tuple[float, float] = (60, 200)  # (100, 160)
...
class RingsLocationConfig(BaseConfig):
    ...
    over_threshold_ids_reported: list[int] | None = None  # 发送过位移告警信息的超过阈值的靶标 id 集合
    loss_ids_reported: list[int] | None = None  # 发送过丢失告警信息的超过阈值的靶标 id 集合
