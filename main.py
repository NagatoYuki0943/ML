...
from adjust_camera import adjust_exposure_full_res_for_loop
...
def main() -> None:
    ...
    # #------------------------------ 调整曝光2 ------------------------------#
    # adjust_exposure_full_res_for_loop(left_camera_queue, left_boxes[0])
    # # 调整曝光后，清空队列，清除调整曝光前的图片
    # clear_queue(left_camera_queue, right_camera_queue)
    # _, left_image, _ = left_camera_queue.get()
    # cv2.imwrite(save_dir / "left_image_adjust_exposure_full_res_for_loop.jpg", left_image)
    # _, right_image, _ = right_camera_queue.get()
    # cv2.imwrite(save_dir / "right_image_adjust_exposure_full_res_for_loop.jpg", right_image)
    # #------------------------------ 调整曝光2 ------------------------------#
