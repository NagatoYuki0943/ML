...
stereo_calibration = StereoCalibration(
    StereoCalibrationConfig.getattr("K"),
    StereoCalibrationConfig.getattr("dist_coeffs"),
)
                    ...
                    if target_number > camera0_got_target_number or target_number == 0:
                        ...
                        try:
                            ...
                            if (
                                ...
                                loss_ids_reported: list | None = (
                                    RingsLocationConfig.getattr("loss_ids_reported")
                                )
                                ...
                                logger.info(
                                    f"loss_ids remove has been reported ids: {loss_ids}"
                                )
                                ...
                                RingsLocationConfig.setattr(
                                    "loss_ids_reported", loss_ids_reported
                                )
                                logger.info(f"loss_ids_reported: {loss_ids_reported}")
                                # -------------------- 移除重复丢失告警消息 -------------------- #

                                if len(loss_ids) > 0:
                                    # ⚠️⚠️⚠️ 暂时关闭 camera0 模板匹配, 并修改已有的靶标记录 ⚠️⚠️⚠️
...
class Send:
    ...
    def send_temperature_control_msg(
        ...
        logger.info(
            f"send camera: {camera}, temperature: {temperature}, temperature control msg"
        )
        # {
        #     "cmd": "adjusttempdata",
        #     "camera": "2"
    ...
    @staticmethod
    def send_stop_temp_control_msg(camera: int = 0):
        """停止温控命令"""
        global is_temp_stable
        ...
        serial_send_queue.put(send_msg)
        logger.success(f"send camera: {camera} stop temperature control msg success")
        is_temp_stable = False
